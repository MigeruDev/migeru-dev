...
class PerlinNoise {
  ...
  public noise(x: number, y: number, z: number): number {
    ...
    return (res + 1) / 2 // Return in range [0, 1]
  }
}
...
export default function DynamicBackground({
  ...
  useEffect(() => {
    ...
    const gridSize    = 12    // px per character
    const noiseScale  = 0.05  // smaller = more “zoomed in”
    const timeScale   = 0.0001
    const octaves     = 4
    const persistence = 0.5

    // **dither-style glyph palette**
    const palette = [" ", "·", "∶", "░", "▒", "▓"]
    ...
    function draw(timestamp: number) {
      ...
      // 2) dark overlay to tone down the pattern
      ctx.fillStyle = "rgba(0, 10, 1, 0.9)"
      ctx.fillRect(0, 0, width, height)

      // 3) glyph style (still neon-green but subtler)
      ctx.fillStyle    = "rgba(34,197,94,0.4)"
      ctx.font         = `${gridSize}px monospace`
      ctx.textBaseline = "middle"
      ctx.textAlign    = "center"

      // 4) fractal Brownian motion noise
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          const idx = Math.min(Math.floor(h * palette.length), palette.length - 1)
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      {/* 4-stop dark-green gradient exactly as before */}
      <canvas
        ref={canvasRef}
